 ...
 * When returns a `Promise` the result of a promise must be a valid `HttpResponse`
 * @example
...
export type ResponseInterceptorFn = (utils: IInterceptorUtils) =>
  IHttpResponse<unknown> | IHttpErrorResponse | Promise<IHttpResponse<unknown> | IHttpErrorResponse>;
  ...
  *             resolve(utils.fn.response(utils.url, 200, { count }));
  *           })();
  *         })
  *     },
