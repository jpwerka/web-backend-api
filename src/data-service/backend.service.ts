...
export abstract class BackendService {
  ...
  private adjustInterceptor(requestInterceptor: IRequestInterceptor): void {
    ...
    if (requestInterceptor.query) {
      ...
      if (typeof query === 'string') {
        ...
        params = new Map(cloneDeep(Array.from(query)));
      } else {
        params = new Map(cloneDeep(Array.from(Object.keys(query).map(key => [key, [query[key]]]))));
      }
  ...
  private get(
    ...
    response$ = this.get$(collectionName, id, query, url, undefined, undefined);
    return this.addDelay(response$, this.config.delay);
  }
  ...
  abstract delete$(collectionName: string, id: string, url: string): Promise<IHttpResponse<null>>;
  ...
  private interceptResponse(interceptor: IRequestInterceptor, utils: IInterceptorUtils):
    IHttpResponse<unknown> | IHttpErrorResponse | Promise<IHttpResponse<unknown>> | undefined {
    let response: IHttpResponse<unknown>;
    if (interceptor.response) {
      if (interceptor.response instanceof Function) {
        ...
        response = cloneDeep(interceptor.response);
      }
    }
    return response;
  }
